...
with tf.name_scope('Conv2'):
    kernel_size_2 = 5
    weight_2 = tf.Variable(tf.truncated_normal([kernel_size_2, kernel_size_2, size_21, size_2], stddev=5e-2))
    kernel_2 = tf.nn.conv2d(norm_1, weight_2, [1, 1, 1, 1], padding='SAME')
    bias_2 = tf.Variable(tf.constant(0.1, shape=[size_2]))
...
with tf.name_scope('FCL'):
    weight_label = tf.Variable(tf.truncated_normal([30, size_label], stddev=1 / 30.0))
    bias_label = tf.Variable(tf.constant(0.0, shape=[size_label]))
    local_label = tf.nn.relu(tf.add(tf.matmul(logits_attri, weight_label), bias_label))

with tf.name_scope('LogitL'):
    weight_logits_label = tf.Variable(tf.truncated_normal([size_label, 230], stddev=1 / float(size_label)))
    bias_logits_label = tf.Variable(tf.constant(0.0, shape=[230]))
    logits_label = tf.add(tf.matmul(local_label, weight_logits_label), bias_logits_label)
